...
int main() {
	...
	hOut = (float*)malloc((N/NUM_OF_THREADS_PER_BLOCK)*sizeof(float));
	refOut = (float*)malloc((N/NUM_OF_THREADS_PER_BLOCK)*sizeof(float));
	gpuErrchk(cudaMalloc((void**)&dA, N*sizeof(float)));
	gpuErrchk(cudaMalloc((void**)&dOut, (N/NUM_OF_THREADS_PER_BLOCK)*sizeof(float)));
	randomize(hA, N);
	gpuErrchk(cudaMemcpy(dA, hA, N*sizeof(float), cudaMemcpyHostToDevice));

	hReduce(hA, refOut, N);

	dim3 gridDim(N/NUM_OF_THREADS_PER_BLOCK);
	dim3 blockDim(NUM_OF_THREADS_PER_BLOCK);
	reduce3<<<gridDim, blockDim, NUM_OF_THREADS_PER_BLOCK*sizeof(float)>>>(dA, dOut);
	gpuErrchk(cudaPeekAtLastError());
	gpuErrchk(cudaMemcpy(hOut, dOut, (N/NUM_OF_THREADS_PER_BLOCK)*sizeof(float), cudaMemcpyDeviceToHost));

	if (check(refOut, hOut, (N/NUM_OF_THREADS_PER_BLOCK)))
		printf("Pass.\n");
	else
		printf("WA.\n");
...
void hReduce(float *hA, float *refOut, int size) {
	for (int i = 0; i < size/NUM_OF_THREADS_PER_BLOCK; i++) {
		refOut[i] = 0;
		for (int j = 0; j < NUM_OF_THREADS_PER_BLOCK; j++) {
			refOut[i] += hA[i * NUM_OF_THREADS_PER_BLOCK + j];
		}
	}
}